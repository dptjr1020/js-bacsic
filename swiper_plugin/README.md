...
`</title> **CDN연결파일들** reset, css,roquf JS 등 연결마무리`
...
1. `html`파일 내에서 슬라이드 삽입 위치에 다음과 같이 태그작성
    ...
    `div.swiper` : swiper 플러그인 시작 위치(js연결지점)
    `div.swiper-wrapper` : 슬라이드를 부모역할(flex내장)
...
3.프로젝트에 디자인에 따라 필요한 투가 ㅐ드룸를  섪
    `div.swiper-slide` 안에 `a, h, p, table, formm, div`등
4. CSS 선택자 준비(개별, 공통용 모두)
    **주의사항**
    * `swiper-wrapper, swiper-slide`
        `display, width, height, position` 적용하지 않기
    `swiper` 제일 바깥쪽 swiper시작태그한테만 크기 적용
    * `swiper-slide` 안 3번 목적으로 만든 태그들은 속성자유
5. JS DOM 변수 준비 -> swiper 연결
    * `const 변수명 = document.querySelecton(대상)`
    * `const 변수명 = new Swiper(스와이프적용변수명, {옵션:값, 옵션:값});`
## Swiper {option:value,} 종류와 뜻 
* `loop:true/false,` : 무한반복재생
* `direction:`vertical/horizontal`,` : 회전방향 수직/수평
* `autoplay:{}` -> delay-정지시간(1000==1초), pauseOnMouseEnter-마우스 들어오면 일시정지, disableOnInteraction-상호작용 후 멈출건지(false가 움직임 기본)
* `effect:cube,fade,flip` : 전환 애니메이션 효과 종류
* `mousewheel:true/false,` : 마우스휠로 슬라이드 넘기기 가능하게 할건지
## Swiper 선택요소(페이지번호, 이전다음버튼, 스크롤바) 주의사항
* 기본 작성 위치는 `.swiper-wrapper`의 다음 형제 위치
    * `swiper`의 첫 번째 자식(swiper-wrapper) 두 번째 자식(선택요소들)
...
### 기존Swiper-slide 외부 바깥쪽으로 안 겹치는 디자인을 하고싶을 때
* 태그 위치 `div.swiper` 다음 형제 위치로 생성
* `div.swiper`와 생성된 서낵태그를 묶어주는 추가 태그 생성(디자인에 따라 다름)
### 교차 여부에 따라 태그 위리츷 다르게 해야하는 이유
* `div.swiper`에 기본 CSS로 `overflow:hidden` 있으므로 그 값을 `visible`로 바꾸면 선택요소 뿐 아닌 `swiper-slide`들도 모두 보이기 때문에 원하는 레이아웃을 만들 수 없음.
## Swiper 안에 추가 Swiper 생성 시 안쪽 Swiper마우스드래그 안될 때 해결방법
