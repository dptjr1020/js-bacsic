...
for(let i=0; i<7; i++){
    console.log(i);
    //요소.이벤트함수(이벤트종류,이벤트했을 때 실행함수)
    navA[i].addEventListener('click',function(){
        console.log('클릭확인')
        navA[i].style.backgroundColor = '#ff0'
    })
    //나머지 연산자로 결과가 0,1에따라 홀, 짝 구분
    // if(i%2==1) navA[i].style.backgroundColor = '#ff0'
    // else navA[i].style.backgroundColor = 'rgb(152, 246, 155)'
    // if(i<4) navA[i].style.backgroundColor = '#ff0'
    // else navA[i].style.backgroundColor = 'rgb(152, 246, 155)'
    // if(i>3) navA[i].style.backgroundColor = 'rgb(152, 246, 155)'
...
for(let i=0; i<2; i++){
    ...
    gnb[i].addEventListener('mouseout',function(){
        ...
        gnb[i].style.background = 'none'//값을 none이라고 쓸 땐 background로 속성 넣기
